...
class SerialTransformerDecoderLayer(nn.Module):
    """
    Args:
      d_model (int): the dimension of keys/values/queries in
          :class:`MultiHeadedAttention`, also the input size of
...
class SerialTransformerDecoder(DecoderBase):
    ...
    def __init__(self,
        ...
        self._coverage = coverage_attn
        self.layer = nn.ModuleList(
            [SerialTransformerDecoderLayer(d_model,
                                     heads,
                                     d_k,
                                     d_v,
                                     d_ff,
    ...
    def init_state(self, src_len, max_len, subtree_len, subtree_max_len):
        """Initialize decoder state."""
        state = dict()
        state["src_len"] = src_len  # [B]
        state["src_max_len"] = max_len  # an integer
        state['subtree_len'] = subtree_len
        state['subtree_max_len'] = subtree_max_len
        state["cache"] = None
        return state
    ...
    def forward(self,
        ...
        src_pad_mask = ~sequence_mask(state["src_len"],
                                      max_len=state["src_max_len"]).unsqueeze(1)
        subtree_pad_mask = ~sequence_mask(state["subtree_len"],
                                      max_len=state["subtree_max_len"]).unsqueeze(1)
        tgt_pad_mask = tgt_pad_mask.unsqueeze(1)  # [B, 1, T_tgt]
