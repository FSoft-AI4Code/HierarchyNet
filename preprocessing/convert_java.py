    ...
    while queue:
        ...
        if current_node.type in ['identifier', 'type_identifier']:
            ...
            if len(sub_tokens) > 1:
                ...
                current_node_json['node_token'] = current_node_json['node_token'] #+ '</w>'
        ...
        if current_type == 'method_declaration':
            ...
            if current_type in ['identifier', 'subscript_expression', 'field_expression']: # a variable is used as an expression
                if parent['node_type'] in ['if_statement', 'while_statement', 'for_statement', 'do_statement', 'switch_statement', 'function_definition', 'synchronized_statement']: # only accept in control statement
                    stmt = [deepcopy(root)]
                    root['children'] = []
                elif  current_type in ['identifier']:
                    for child in children:
                        stmt.extend(fn(child, root))
            else:
                stmt = [deepcopy(root)]
...
def bfs_build_graph(root, stmts, stmt_indices):
    ...
    while queue:
        ...
        for child in children:
            # print(child)
            # if child['node_type'] in ['sub_identifier', 'sub_type_identifier']: continue
            child_index = child['node_index']
            if child_index != -1:
...
def add_data_flow_edges(root, G, stmt_indices, stmt_id2names, stmt_id2stmts, scope, *, brk_scope = None, con_stmt_id = [-1]):
    ...
    if is_stmt:
        ...
        current_scope = scope
        if root['node_type'] == 'method_declaration':
            for name in stmt_id2names[node_index]:
                current_scope[name] = [node_index]
        elif root['node_type'] == 'assignment_expression':
            # for name in stmt_id2names[node_index]:
            #     current_scope[name] = [node_index]
...
def convert_into_java_graph(tree, text, tokenize_fn,  max_flattened_nodes = 500):
    ...
    removed_node_ids = []
    if len(flattened_nodes) > max_flattened_nodes:
        _tree_stmts  = []
        max_tree_dfs_ids = list(map(lambda x: x['max_dfs_id'], tree_stmts))
        _stmt_indices = []
        for i, max_dfs_ids in enumerate(max_tree_dfs_ids):
            if max_dfs_ids > max_flattened_nodes:
                removed_node_ids.append(tree_stmts[i]['tree_index'])
            else:
                _tree_stmts.append(tree_stmts[i])
                _stmt_indices.append(stmt_indices[i])
        tree_stmts = _tree_stmts
        stmt_indices = _stmt_indices

        _ast_node_ids = []
        _ast_dfs_ids = []
        for ast_id, dfs_id in zip(ast_node_ids, ast_dfs_ids):
            if dfs_id < max_flattened_nodes:
                _ast_node_ids.append(ast_id)
                _ast_dfs_ids.append(dfs_id)
            else:
                removed_node_ids.append(ast_id)
        ast_node_ids = _ast_node_ids
        ast_dfs_ids = _ast_dfs_ids
        flattened_nodes = flattened_nodes[:max_flattened_nodes]
    ...
    return refactor_json, flattened_nodes, ast_node_ids, ast_dfs_ids, tree_stmts, stmt_indices, G
